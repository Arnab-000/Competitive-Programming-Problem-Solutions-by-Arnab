...
//[Key Idea: Lets take the minimum number. Now gcd(minimum, any number) will be minimum number if and only if the second number is divisible by minimum. So if a number is not divisible by minimum then their gcd will be 1 and we can not perform any swap operation on them. So we just need to check if any number on which we can't perform operation [which is not divisible by minimum number] is on the right position on sorted array. If it is not on the correct position on sorted array then we can not swap the number and bring it on its expected position then the answer is NO , otherwise answer is always YES]
