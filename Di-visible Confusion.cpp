...
// [Key idea: Lets start with a question, when it is impossible to remove a number ? The answer is simple, we can not remove a number when we have no such position where number % position != 0. According to question, we can always remove a number if it is not divisible by its next position. So we can move a number to its left only. For example, if we have 1 2 3 4 , then we can move the number 3 to left, as we can remove 2 then array is 1 3 4. So for any number we need to make sure that, we have at least one such position where it is not divisible by its position to its left. So for any index, a number can not be moved to anywhere to its left if lcm(2...index+1) divides the number. So we just need to make sure that, there does not exist any number which can not be moved to its left. If we find such number, then the answer is NO, otherwise the answer is YES. ]
...
void solve()
{
    ...
    int n;
    cin >> n;
    vi v(n + 10), plcm(n + 10, 1);
    for (int i = 1; i <= n; i++) {
        cin >> v[i];
    }
    for (int i = 1, j = 2 ; i <= n; i++, j++) {
        plcm[i] = lcm(plcm[i - 1], j);
    }
    for (int i = 1; i <= n; i++) {
        if (v[i] % plcm[i] == 0) {
            cout << "NO\n";
