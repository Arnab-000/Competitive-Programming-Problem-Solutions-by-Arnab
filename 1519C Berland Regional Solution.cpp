...
// Time Limit: 2000 ms
// Author: Catalyst71
// Key Idea: Just implementation and nothing more. Iterate over university to make it in give timeframe.
...
void solve()
{
    ...
    fx(ps){
    	for(int temp = 1; temp <= (int)(x.second.size()); temp++){
    		k[temp] += x.second[x.second.size() - (x.second.size() % temp) - 1];
    	}
    }
