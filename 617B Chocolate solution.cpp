...
// [Key Idea : The idea is to find the number of 0's between any two one. The partition we can do is, number of 0's + 1 . 
// Lets say we have 10001 so, total partition will be 4. like:
// 10| 001
// 100| 01
// 1000 | 1
// Number of zeros
...
// Now we just need to multiply all the possibilities to get final answer.
// ]
...
void solve()
{
    ...
    for (int i = 0; i < n; i++) {
        cin >> v[i];
        if (v[i]) onePos.emplace_back(i);
    }
    if (!onePos.size()) {
        cout << 0 << "\n";
    }
    else if ((int)onePos.size() == 1) {
        cout << 1 << "\n";
    } else {
        long long ans = 1;
