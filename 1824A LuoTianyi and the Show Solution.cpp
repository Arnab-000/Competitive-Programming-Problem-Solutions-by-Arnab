...
// [Problem Link : https://codeforces.com/problemset/problem/1824/A]
...
void solve()
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        else specific.insert(x);
    }
    vector<int> kept;
    for (auto i : specific) kept.push_back(i);
    int fixed = kept.size();
    int ans = max(fixed + l, fixed + r);
    ans = min(ans, m);
    for (int i = 0; i < kept.size(); ++i) {
        int max_l = min(kept[i] - i - 1, l);
        int max_r = min(m - kept[i] - ((int)kept.size() - 1 - i), r);
        ans = max(ans, fixed + max_l + max_r);
    }
    cout << ans << "\n";
