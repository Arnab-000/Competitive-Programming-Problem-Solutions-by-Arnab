...
//[Key Idea : As we need to find that number which divides all the number of array, we can simply calculate the GCD of whole array, because GCD of whole array will divide each element in the array. after calculating cumulative GCD we can find the numbers that divides the GCD. So our final answer is the number of divisor of cumulative gcd of whole array.]
...
void solve()
{
    ...
    ll cumGcd = v[0];
    for (int i = 0; i < n; i++) {
        cumGcd = gcd(cumGcd, v[i]);
    }

    ll ans = 0;
    for (ll i = 1; i <= sqrt(cumGcd + 1); i++) {
        if (cumGcd % i == 0) {
            if (cumGcd / i == i) ans++;
            else ans += 2 ;
        }
