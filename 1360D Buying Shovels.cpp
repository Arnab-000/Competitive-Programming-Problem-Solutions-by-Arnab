...
//[Key Idea : So he will buy exactly n shovels. So whatever package he chooses it must be a divisor of n. And we need to minimize the number of packages so we need to maximize the divisor so that the number of package is minimum. Lets first find all the divisors of n and sort them in Descending order. Now we just have to check the largest divisor which is <= k]
