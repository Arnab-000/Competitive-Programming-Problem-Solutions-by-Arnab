...
// [Problem link : https://codeforces.com/problemset/problem/1819/A]

// [Key idea: as we need to update mex just by 1 we need to remove all the occurence of nextMex. lets say next mex is present between position a and b. now it can be possible that any number which is less than mex is present in the segment a..b then if we remove a segment a...b then ocurrence this number which is less than mex may become 0. if this happens then value of mex will decrease.

// so we need to remove a segment from first position of nextMex to  last position of nextMex and also make sure that all number which are less than current mex is present in array even after removal of segment]
...
void solve()
{
    ...
    if (mex == n) cout << "No\n";
    else if (!mp[nextMex]) cout << "Yes\n";
    else {
        int fpos = -1, lpos = -1;

        for (int i = 0 ; i < n; i++) {
            if (nextMex == v[i] && fpos == -1) {
                fpos = i;
            }
            if (nextMex == v[i]) {
                lpos = i;
