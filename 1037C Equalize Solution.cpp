...
//[Key Idea : How much we must need ? if there are n difference in both string then, we must need n number of operations to perform. Now out target is to minimize it. We can minimize it by finding the number of such operation, where we can fix two position with one operation. that is lets say in first string we have 01 and target string has 10 then with one operation we can fix them. So the idea is to calculate the must needed operation, and then whenever we can find such operation where one swap can fix two positions then we will decrease count.]
