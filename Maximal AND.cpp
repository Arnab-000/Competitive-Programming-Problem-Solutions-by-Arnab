...
// [Key Idea : Key idea to make all the bit 1 of any position between 1 and 31 inclusive as an integer number can be represented in 32 bits in Binary. And to maximize the answer we need to make all bit 1 to the leftmost possible. ]
...
ll binaryToDecimal(vector<int> final)
{
    ll dec_num = 0;
    ll power = 0 ;
    ll n = final.size() ;
...
void solve()
{
    ...
    int k; cin >> k;
    vector<int> v(n);
    vector<vector<int>> Binary(n);

    for (int i = 0; i < n; i++) {
        cin >> v[i];
        Binary[i] = decToBinary(v[i]);
    }
