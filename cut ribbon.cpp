...
// [Key Idea : Let's just go with bruteforce but optimize it by calculating the most division can be performed by smallest number]
        ...
        for (int j = 0 ; j <= 4000; j++) {
            for (int k = 0 ; k <= 4000; k++) {
                if (i * v[0] + j * v[1] + k * v[2] == n) {
                    ans = max(ans, i + j + k);
                } else if (i * v[0] + j * v[1] + k * v[2] > n) break;
            }
        }
    }
    cout << ans;
